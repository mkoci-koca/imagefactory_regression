...
#check if aeolus-cleanup removes directory. /var/tmp and /var/lib/iwhd/images
    ...
    if os.system("grep -i \"error\\|failed\" " +  TmpFile + "| grep -v \"failed to create directory: File exists\"") == SUCCESS:
        ...
        print outputtmp     
        return False     
    print "Checking if there is any visible password "+password_yaml['classes']['aeolus::profiles::vsphere']['instances']['default']["password"]+" in error log of image factory"
    if os.system("grep -i \""+password_yaml['classes']['aeolus::profiles::vsphere']['instances']['default']["password"]+"\" " + LogFileIF) == SUCCESS:
        print "Found "+password_yaml['classes']['aeolus::profiles::vsphere']['instances']['default']["password"]+":"
        outputtmp = os.popen("grep -i \""+password_yaml['classes']['aeolus::profiles::vsphere']['instances']['default']["password"]+"\" " + LogFileIF).read()
        print outputtmp
        print "See the output from log file " + LogFileIF + ":"
        print "======================================================"
        outputtmp = os.popen("cat " + LogFileIF).read()
